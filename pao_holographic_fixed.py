"""
Holographic Lubricant Solver - FIXED VERSION by Tamaki
...
def solve_homotopy_pao_ultimate(
    ...
    Lambda_vals = [
        compute_Lambda_field_pao_ultimate(
            params_warmup, d.temperature, d.pressure, exp_data
        )
        for d in exp_data
    ]
...
"""
    ...
    plt.tight_layout()
    plt.savefig('pao_holographic_results_fixed.png', dpi=150, bbox_inches='tight')
    print("\n結果図を保存: pao_holographic_results_fixed.png")
    plt.show()
...
def run_pao_holographic_ultimate():
    """PAOホログラフィック実験（環修正版）"""
    print("="*60)
    print("PAO Holographic Analysis - Fixed by Tamaki")
    print("完全修正版・自己無撞着スケール")
    print("="*60)
    ...
    print("  - Lambda_scale = median(burnout K/V) × (1-ρ)")
    print("  - Λ ≈ 1/(1-ρ) at burnout")
    ...
    if len(Lambda_safe_for_boundary) > 0 and len(Lambda_danger_for_boundary) > 0:
        boundary_threshold = (np.max(Lambda_safe_for_boundary) + 
                            np.min(Lambda_danger_for_boundary)) / 2.0
    else:
    ...
    Sigma = extract_critical_boundary_pao(
        params_opt, T_range, P_range, exp_data, Lambda_crit=boundary_threshold
    )
    ...
    if len(Sigma) > 0:
        # ★修正：exp_data引数を追加
        Ξ = compute_boundary_info_packet_pao(params_opt, Sigma, exp_data)
        ...
        # 保存
        with open('Xi_boundary_pao_fixed.json', 'w') as f:
            Ξ_serializable = {}
        ...
        print("  保存: Xi_boundary_pao_fixed.json")
    
    # 最終パラメータ保存
    final_params = {**PAO_PARAMS_FROZEN, **params_opt}
    with open('pao_holographic_params_fixed.json', 'w') as f:
        json.dump(final_params, f, indent=2)
    print("\n最終パラメータ保存: pao_holographic_params_fixed.json")
